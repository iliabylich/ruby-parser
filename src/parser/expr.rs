...
where
    C: Constructor,
{
    pub(crate) fn try_expr(&mut self) -> Result<Box<Node>, ParseError> {
...
fn try_bang_command_call<C: Constructor>(parser: &mut Parser<C>) -> Result<Box<Node>, ParseError> {
...
fn try_arg_assoc_p_expr_body<C: Constructor>(
    parser: &mut Parser<C>,
...
fn try_arg_in_p_expr_body<C: Constructor>(parser: &mut Parser<C>) -> Result<Box<Node>, ParseError> {
    let arg = parser.try_arg()?;
