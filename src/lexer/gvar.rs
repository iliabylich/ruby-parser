...
pub(crate) fn parse_gvar<'a>(buffer: &mut Buffer<'a>) -> Token<'a> {
...
pub(crate) fn lookahead_gvar<'a>(buffer: &Buffer<'a>, start: usize) -> LookaheadGvarResult<'a> {
    let mut ident_start = start + 1;

    let empty_gvar_name = || {
        LookaheadGvarResult::EmptyVarName(Token(
            TokenValue::tGVAR(buffer.slice(start, start + 1)),
            Loc(start, start + 1),
        ))
    };

    let invalid_gvar_name = |end: usize| {
        LookaheadGvarResult::InvalidVarName(Token(
            TokenValue::tGVAR(buffer.slice(start, end)),
            Loc(start, end),
        ))
    };
    ...
    match buffer.byte_at(start + 1) {
            ...
            match buffer.byte_at(start + 2) {
                ...
                _ => {
                    ...
                    return LookaheadGvarResult::Ok(Token(
                        TokenValue::tGVAR(buffer.slice(start, start + 2)),
                        Loc(start, start + 2),
                    ));
                }
            }
        ...
        Some(b'&') | Some(b'`') | Some(b'\'') | Some(b'+') => {
            return LookaheadGvarResult::Ok(Token(
                TokenValue::tBACK_REF(buffer.slice(start, start + 2)),
                Loc(start, start + 2),
            ));
        }
        ...
        Some(b'1'..=b'9') => {
            ...
            return LookaheadGvarResult::Ok(Token(
                TokenValue::tNTH_REF(buffer.slice(start, end)),
                Loc(start, end),
            ));
        }
...
assert_lex!(
    ...
    tGVAR(b"$_\xD0\xB0\xD0\xB1\xD0\xB2"),
    0..8 // foo
);
assert_lex!(
...
assert_lex!(
    ...
    tGVAR(b"$\xD0\xB0\xD0\xB1\xD0\xB2"),
    0..7
);
assert_lex!(
